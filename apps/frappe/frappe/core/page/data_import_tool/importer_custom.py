...
from frappe.core.page.import_sales_report.import_sales_report import get_data_keys
...
def upload(rows = None, submit_after_import=None, ignore_encoding_errors=False, overwrite=None,
        ...
        try:
            ...
            if parentfield:
                ...
                if overwrite and doc["name"] and frappe.db.exists(doctype, doc["name"]):
                    ...
                    #MARLO 20161028
                    #check if stock levels are affected, if not manually update stocks
    ...
    #def check_and_remove_duplicates():
        # via sql
        # determine which fields / formula to check for double entry
        # log('Cleaning double-entries')
