...
from erpnext.selling.doctype.sales_invoice.pos import update_multi_mode_option
...
class SalesInvoice(SellingController):
    def __init__(self, arg1, arg2=None):
        ...
        self.update_stock = 1   #MARLO updated 20161021
        self.account_for_change_amount = "Cash - APT" #MARLO updated 20170203
        self.status_updater = [{
        ...
        for d in items:
            if not d.item_code:
                ...
                if not d.sku:
                    ...
                    item_code = frappe.db.get_value("SKU", d.sku, "item")
                    if not item_code:
                        msgprint(_("SKU Not Valid at Row No {0}").format(d.idx), raise_exception=True)
                                        
                    item_name = frappe.db.get_value("Item", item_code, "item_name")
                    
                    d.item_code = item_code
                    d.item_name = item_name
                    d.description = item_name
    

    def validate_item_code(self):
        self.get_item_code_from_sku(self.get('items'))
        for d in self.get('items'):               
            if not d.item_code:
    ...
    def validate_warehouse(self):
        ...
        the_customer = frappe.get_doc("Customer", self.customer)
    ...
    def set_customer_from_store_code(self):
        # MARLO 20161216
        if ((not self.customer) and self.customer_group and self.store_code):
            customer, customer_name = frappe.db.get_value(
                "Customer", 
                {"customer_group": self.customer_group, "store_code":self.store_code }, 
                ["name", "customer_name"]
            )
            self.customer = customer
            self.title = self.customer_name = customer_name
            
    # MARLO 20161221
    def validate_amounts(self):
        create_zero_amount_items_for_returns = False
        if not self.grand_total:
            sum_total = 0
            for d in self.get("items"):
                if d.qty < 0 :
                    create_zero_amount_items_for_returns = True
                if d.qty == 0 :
                    d.amount = 0
                else :
                    d.rate = abs(d.amount / d.qty)
                d.base_rate = d.rate
                d.net_amount = d.base_net_amount = d.base_amount = d.amount
                sum_total += d.net_amount
                
            self.base_total = self.base_grand_total = self.base_net_total = self.grand_total = sum_total               
            
        # MARLO 20170203
        if create_zero_amount_items_for_returns :
            # create new invoice
            # add the items with 0 amount and qty
            # save the invoice with 0 amounts
            # get the id of the 0 invoice
            # attach the id of 0 invoice to current invoice
            # set is_return of current invoice
            import copy
            
            zero_invoice = frappe.copy_doc(self, False)
            zero_invoice.name = None
            zero_invoice.grand_total = 0
            zero_invoice.posting_date = self.posting_date
            zero_invoice.posting_time = self.posting_time
            zero_invoice.due_date = self.due_date
            
            for d in zero_invoice.get("items"):
                d.qty = d.amount = 0
            zero_invoice.validate()
            zero_invoice.insert()
            zero_invoice.submit()
            ...
            self.return_against = zero_invoice.name
