...
class Customer(TransactionBase):
    def before_insert(self):
        # MARLO 20161202
        # CHECK PRICE LIST OF CUSTOMER GROUP
        if not self.default_price_list:
            price_list_name = frappe.db.get_value("Price List", {"customer_group": self.customer_group}, 'price_list_name')
            if not price_list_name:
                new_price_list = frappe.new_doc("Price List")
                new_price_list.customer_group = self.customer_group
                new_price_list.selling = 1
                new_price_list.price_list_name = self.customer_group
                new_price_list.insert()
                
                price_list_name = self.customer_group
                
            self.default_price_list = price_list_name
        else:
            if not frappe.db.exists("Price List", self.default_price_list):
                new_price_list = frappe.new_doc("Price List")
                new_price_list.customer_group = self.customer_group
                new_price_list.selling = 1
                new_price_list.price_list_name = self.default_price_list
                new_price_list.insert()
                
        # MARLO 20161024
        # CREATE A WAREHOUSE WITH SAME NAME AND SET IT AS DEFAULT WAREHOUSE
        if self.create_store:
            ...
            if self.ppp_location_code:
                store.ppp_location_code = self.ppp_location_code
            
            store.insert()
            
            self.default_warehouse = store.name
    ...
    def autoname(self):
        ...
        if self.store_code:
            self.name = self.customer_group + "-" + self.store_code
        else:
