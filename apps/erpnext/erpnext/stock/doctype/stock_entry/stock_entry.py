...
class StockEntry(StockController):
    ...
    def after_insert(self) :
        if not self.ppp_ref_no :
            self.ppp_ref_no = make_autoname( self.ppp_series + '.#####')
        self.ppp_voucher_name = self.name
        self.save()
        
    # MARLO 20170330
    def validate_same_items(self) :
        item_list = []
        for item in self.get("items") :
            if(item.item_code in item_list) :
                frappe.throw(_("Multiple Entries of {0} is not allowed").format(item.item_code))
    ...
    def validate_amendment(self) :
        if self.amended_from :
            ...
            if not frappe.db.exists("Stock Entry", {"name": self.amended_from}) :
                frappe.throw(_("Cannot find Voucher {0}").format(self.amended_from))
            base_voucher = frappe.get_doc("Stock Entry", {"name": self.amended_from})
            if  not self.naming_series == base_voucher.naming_series or \
            not self.from_warehouse == base_voucher.from_warehouse or \
    ...
    def update_pending_count(self, clear_pending=False):
        for item in self.get("items"):
            if self.from_warehouse :                    
                if not frappe.db.exists('Bin', {"warehouse": self.from_warehouse, "item_code": item.item_code}):
                    bin_from = frappe.get_doc({
                        "doctype": "Bin",
                        ...
                        "ppp_pending_qty": -item.qty
                    })
                    bin_from.insert()
                else :
                    bin_from = frappe.get_doc('Bin', {"warehouse": self.from_warehouse, "item_code": item.item_code})
                    bin_from.ppp_pending_qty -= item.qty * (clear_pending and -1 or 1)
                    bin_from.save()
                
            if self.to_warehouse :
                if not frappe.db.exists('Bin', {"warehouse": self.to_warehouse, "item_code": item.item_code}):
                    bin_to = frappe.get_doc({
                        "doctype": "Bin",
                    ...
                    bin_to = frappe.get_doc('Bin', {"warehouse": self.to_warehouse, "item_code": item.item_code})
                    bin_to.ppp_pending_qty += item.qty * (clear_pending and -1 or 1)
                    bin_to.save()
    ...
    def validate_purpose(self):
        valid_purposes = ["Material Transfer", "Adjustments", "Pullout"]
    ...
    def validate_warehouse(self):
        ...
        if(not self.from_warehouse and not self.to_warehouse) :
    ...
    def update_stock_ledger(self):
        ...
        # MARLO 20170329
        is_direct_adjustment = False
        if self.amended_from :
            if frappe.db.exists("Stock Entry", {"name": self.amended_from, "docstatus": 1}) :
                is_direct_adjustment = True
                
        if not is_direct_adjustment :
            ...
            for d in new_items:
                if cstr(self.from_warehouse):
                    orig_qty = 0
                    orig_d = next((item for item in orig_items if item.item_code == d.item_code), None)
                    if orig_d :
                        orig_qty = orig_d.transfer_qty                        
                    difference_qty = orig_qty - d.transfer_qty
                    sl_entries.append(self.get_sl_entries(d, {
                        "warehouse": cstr(self.from_warehouse),
                        "actual_qty": difference_qty,
                        "incoming_rate": 0
                    }))
            ...
            new_item_codes = list(d.item_code for d in new_items)
            items_removed = list(d for d in orig_items if d.item_code not in new_item_codes)
            for d in items_removed:
                if cstr(self.from_warehouse):
                    sl_entries.append(self.get_sl_entries(d, {
                        "warehouse": cstr(self.from_warehouse),
                        "actual_qty": d.transfer_qty,
                        "incoming_rate": flt(d.valuation_rate)
                    }))
            for d in items_removed:
                if cstr(self.to_warehouse):
...
def ppp_confirm_dc(stock_entry_tr, date_confirmed):
    ...
    if stock_entry.ppp_is_adjustable and not stock_entry.has_amendments():
        ...
        stock_entry.save()
        frappe.msgprint("Confirmed in DC")
    else :
        frappe.msgprint("Changes not Allowed")

# MARLO 20170321        
@frappe.whitelist()
def ppp_confirm_store(stock_entry_tr, date_confirmed):
    if not validate_date(date_confirmed):
        frappe.throw("Please supply a valid date");
        
    stock_entry = frappe.get_doc("Stock Entry", stock_entry_tr)
    if stock_entry.ppp_is_adjustable and not stock_entry.has_amendments():
        for_update = (stock_entry.ppp_confirmed_in_store is None) and True or False
        stock_entry.ppp_confirmed_in_store = date_confirmed
        stock_entry.save()
        if for_update :
            stock_entry.update_pending_count(True)
        frappe.msgprint("Confirmed in Store")
    else :
        frappe.msgprint("Changes not Allowed")
