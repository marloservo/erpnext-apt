...
erpnext.stock.StockEntry = erpnext.stock.StockController.extend({
    ...
    onload_post_render: function() {
        ...
        function get_last_param() {
            var hist_count = frappe.route_history.length;
            if(hist_count > 2) {
                if(frappe.route_history[hist_count-2].length > 3) {
                    return frappe.route_history[hist_count-2][3];
                }
            }
            return "NOTHING";
        }
        ...
        //MARLO 20161026
        var list_allowed = ["Material Transfer", "Adjustments", "Pullout"];
        var params = get_last_param();
        if($.inArray(params, list_allowed) > -1) {
            me.frm.set_value("purpose", params);
            this.frm.toggle_enable("purpose", false);
        }
        //////////////
