...
class Warehouse(NestedSet):
    ...
    ''' REVERT - CREATE WAREHOUSE ONLY ON CHECK OF BUTTON IN CREATE CUSTOMER
    def before_insert(self):
        # MARLO 20161017
        # get the Customer Group from the store name
        # create a customer with matching name (store name)
        #   set Default Warehouse for the Customer
        #
        customer_name = self.warehouse_name
    ...
    def after_insert(self):
        ...
        customer = frappe.get_doc("Customer", self.warehouse_name)
        customer.default_warehouse = self.name
        customer.save()
    '''
    
    def autoname(self):
        # MARLO 20161215
        if self.ppp_store_code:
            self.name = self.customer_group + "-" + self.ppp_store_code
        else:
            if self.company:
